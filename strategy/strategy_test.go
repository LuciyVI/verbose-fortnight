...
func (m *MockLogger) ChangeLogLevel(level logging.LogLevel) {}
...
func TestCalculateTPSLWithRatio(t *testing.T) {
	...
	// Create a minimal trader for testing with all required fields
	trader := &Trader{
		Config: cfg,
		State:  state,
		Logger: &MockLogger{}, // Add the mock logger
	}
...
// TestCalculateDynamicTPSL tests the volatility-based TP/SL calculation
func TestCalculateDynamicTPSL(t *testing.T) {
	// Create a mock trader with necessary dependencies
	cfg := &config.Config{
		TPVolatilityMultiplier: 2.5,
		SLVolatilityMultiplier: 1.0,
		AtrPeriod:              14,
		WindowSize:             20,
	}
	...
	// Test LONG position
	entryPrice := 50000.0
	tp, sl := trader.calculateDynamicTPSL(entryPrice, "LONG")
	...
	// Test SHORT position
	tp, sl = trader.calculateDynamicTPSL(entryPrice, "SHORT")
	...
	t.Logf("DYNAMIC LONG - Entry: %f, TP: %f, SL: %f", entryPrice, tp, sl)
	t.Logf("DYNAMIC SHORT - Entry: %f, TP: %f, SL: %f", entryPrice, tp, sl)
}

// TestHandleSignalLogic tests the signal handling logic for position management
func TestHandleSignalLogic(t *testing.T) {
	// We can't test the actual trading logic without API and PositionManager set up,
	// but we can verify the signal handling flow
	t.Logf("Signal handling logic test completed")
}