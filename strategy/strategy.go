...
func NewTrader(apiClient *api.RESTClient, cfg *config.Config, state *models.State, logger logging.LoggerInterface, webUI *web_interface.WebUI) *Trader {
	...
	// Initialize the new signal system components
	trader.initializeSignalSystem()

	return trader
}

// initializeSignalSystem initializes all the new signal system components
func (t *Trader) initializeSignalSystem() {
...
func (t *Trader) HandleLongSignal(closePrice float64) {
	...
	normalizedSide := t.PositionManager.NormalizeSide(side)
	newSide := t.PositionManager.NormalizeSide("LONG")

	if !exists {
		// No open position, open new long position
		t.openPosition(newSide, closePrice)
	} else if normalizedSide == newSide {
		// Position in the same direction, only adjust TP/SL
		t.adjustTPSL(closePrice)
		t.Logger.Info("Confirmed LONG signal received: adjusted TP/SL, current position maintained")
	} else {
		// Counter-trend signal, close existing position and open new one
		t.Logger.Info("Counter-trend signal received: closing %s position, opening LONG position", normalizedSide)
...
func (t *Trader) HandleShortSignal(closePrice float64) {
	...
	if !exists {
		// No open position, open new short position
		t.openPosition(newSide, closePrice)
	} else if normalizedSide == newSide {
		// Position in the same direction, only adjust TP/SL
		t.adjustTPSLForShort(closePrice)
		t.Logger.Info("Confirmed SHORT signal received: adjusted TP/SL, current position maintained")
	} else {
		// Counter-trend signal, close existing position and open new one
		t.Logger.Info("Counter-trend signal received: closing %s position, opening SHORT position", normalizedSide)
		t.closeOppositePosition(newSide)
		t.openPosition(newSide, closePrice)
	}
}
...
func (t *Trader) openPosition(newSide string, price float64) {
	...
	var tp, sl float64
	if t.Config.DynamicTP {
		// Use dynamic calculation based on market volatility
		tp, sl = t.calculateDynamicTPSL(entry, newSide)
		t.Logger.Info("Setting dynamic TP/SL based on volatility: TP=%.2f, SL=%.2f", tp, sl)
	} else {
...
// calculateDynamicTPSL calculates TP/SL based on market volatility using multiple indicators
func (t *Trader) calculateDynamicTPSL(entryPrice float64, positionSide string) (takeProfit float64, stopLoss float64) {
	// Calculate multiple volatility measures
	atr := indicators.CalculateATR(t.State.Highs, t.State.Lows, t.State.Closes, t.Config.AtrPeriod)
	if atr <= 0 {
		// Fallback to ATR with default period if calculation fails
		atr = indicators.CalculateATR(t.State.Highs, t.State.Lows, t.State.Closes, 14)
		if atr <= 0 {
			t.Logger.Warning("ATR calculation failed, using standard ATR method")
			return t.calculateTPSLWithRatio(entryPrice, positionSide)
		}
	}

	// Calculate standard deviation as another volatility measure
	stdDev := indicators.StdDev(t.State.Closes)
	if stdDev <= 0 && len(t.State.Closes) > 0 {
		// Calculate standard deviation manually from recent closes
		mean := indicators.SMA(t.State.Closes)
		var sum float64
		for _, c := range t.State.Closes {
			sum += (c - mean) * (c - mean)
		}
		stdDev = math.Sqrt(sum / float64(len(t.State.Closes)))
	}

	// Calculate Bollinger Bands width as another volatility measure
	bbUpper, bbMiddle, bbLower := indicators.CalculateBollingerBands(t.State.Closes, t.Config.WindowSize, 2.0)
	bbWidth := (bbUpper - bbLower) / bbMiddle // Percentage width of the bands

	t.Logger.Debug("Volatility measures - ATR: %.4f, StdDev: %.4f, BBWidth: %.4f", atr, stdDev, bbWidth)

	// Use the highest volatility measure for conservative TP/SL placement
	var volatilityMeasure float64
	switch t.State.MarketRegime {
	case "trend":
		// In trending markets, use ATR as primary volatility measure
		volatilityMeasure = atr
		t.Logger.Debug("Using ATR as volatility measure in trending market")
	case "range":
		// In ranging markets, consider both ATR and Bollinger Bands width
		if bbWidth > 0 && atr > 0 {
			// Combine ATR and BB width to get a more comprehensive volatility measure
			volatilityMeasure = math.Max(atr, bbWidth*entryPrice)
		} else {
			volatilityMeasure = atr
		}
		t.Logger.Debug("Using combined ATR/BB measure as volatility measure in ranging market")
	default:
		// Default to ATR if regime is unknown
		volatilityMeasure = atr
		t.Logger.Debug("Using ATR as default volatility measure")
	}

	// Calculate TP and SL based on volatility-adjusted multipliers
	tpVolMultiplier := t.Config.TPVolatilityMultiplier
	slVolMultiplier := t.Config.SLVolatilityMultiplier
	...
	if positionSide == "LONG" {
		// For LONG positions: TP above entry, SL below entry
		// Add commission to TP to ensure profit after fees, reduce SL to account for commission
		takeProfit = entryPrice + (volatilityMeasure * tpVolMultiplier) + (entryPrice * commission)
		stopLoss = entryPrice - (volatilityMeasure * slVolMultiplier) - (entryPrice * commission)
	} else if positionSide == "SHORT" {
		// For SHORT positions: TP below entry, SL above entry
		// Reduce TP to account for commission, increase SL to account for commission
		takeProfit = entryPrice - (volatilityMeasure * tpVolMultiplier) - (entryPrice * commission)
	...
	if positionSide == "LONG" {
		if takeProfit <= entryPrice {
			takeProfit = entryPrice + (volatilityMeasure * tpVolMultiplier)
		}
		if stopLoss >= entryPrice {
			stopLoss = entryPrice - (volatilityMeasure * slVolMultiplier)
		}
	} else if positionSide == "SHORT" {
		if takeProfit >= entryPrice {
			takeProfit = entryPrice - (volatilityMeasure * tpVolMultiplier)
	...
	t.Logger.Debug("Volatility-based TP/SL calculation - Entry: %.2f, TP: %.2f, SL: %.2f, TP Dist: %.4f, SL Dist: %.4f, Ratio: %.2f:1 (Target TP: %.2f, SL: %.2f)",
		entryPrice, takeProfit, stopLoss, tpDistanceActual, slDistanceActual, ratio, tpVolMultiplier, slVolMultiplier)
...
func (t *Trader) adjustTPSL(closePrice float64) {
	...
	trailingTP := closePrice + atr*1.5
	if trailingTP <= curTP {
		t.Logger.Info("New TP %.2f is not better than current TP %.2f, skipping update", trailingTP, curTP)
		return
	}
	...
	if t.Config.DynamicTP {
		// Use dynamic calculation based on market volatility
		newTP, newSL = t.calculateDynamicTPSL(entry, "LONG")
		t.Logger.Info("Sending dynamic TP/SL update based on volatility: TP %.2f, SL %.2f", newTP, newSL)
	} else {
		// Calculate TP/SL with 2:1 ratio based on 15-minute projection
...
func (t *Trader) adjustTPSLForShort(closePrice float64) {
	...
	trailingTP := closePrice * 0.998
	if trailingTP >= curTP*1.001 { // Protection against small changes
		t.Logger.Info("New TP %.2f is not better than current TP %.2f, skipping update", trailingTP, curTP)
		return
	}
	...
	if t.Config.DynamicTP {
		// Use dynamic calculation based on market volatility
		newTP, newSL = t.calculateDynamicTPSL(entry, "SHORT")
		t.Logger.Info("Sending dynamic TP/SL update based on volatility: TP %.2f, SL %.2f", newTP, newSL)
	} else {
		// Calculate TP/SL with 2:1 ratio based on 15-minute projection
