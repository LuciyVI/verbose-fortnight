...
func connectPrivateWS(apiClient *api.RESTClient, state *models.State) (*websocket.Conn, error) {
	logInfo("Attempting to connect to private WebSocket: %s", cfg.DemoWSPrivateURL)
	...
	logDebug("Sending authentication request to exchange: %v", auth)
	...
	logDebug("Received authentication response from exchange: %s", string(authResp))
	...
	logDebug("Sending subscription request to exchange: %v", subReq)
	...
	logDebug("Received subscription response from exchange: %s", string(subResp))
...
func reconnectPublic(klineTopic, obTopic string, state *models.State) (*websocket.Conn, error) {
	...
	for backoff := 1; ; backoff++ {
		...
		logDebug("Sending subscription request to exchange during reconnection: %v", subReq)
		...
		if err == nil {
			logDebug("Received subscription response from exchange during reconnection: %s", string(msg))
...
func applySnapshot(state *models.State, bids, asks [][]string) {
	logDebug("Received orderbook snapshot from exchange - bids: %d, asks: %d", len(bids), len(asks))
	...
	logDebug("Orderbook snapshot applied - bids: %d, asks: %d", len(state.BidsMap), len(state.AsksMap))
}

// ApplyDelta applies orderbook delta
func applyDelta(state *models.State, bids, asks [][]string) {
	logDebug("Received orderbook delta from exchange - bid updates: %d, ask updates: %d", len(bids), len(asks))
	...
	for _, lv := range bids {
		...
		if size == 0 {
			...
			logDebug("Removed bid level: %s", lv[0])
		} else {
			state.BidsMap[lv[0]] = size
			logDebug("Updated bid level: %s = %f", lv[0], size)
		}
	}
	for _, lv := range asks {
		if len(lv) < 2 {
		...
		if size == 0 {
			...
			logDebug("Removed ask level: %s", lv[0])
		} else {
			state.AsksMap[lv[0]] = size
			logDebug("Updated ask level: %s = %f", lv[0], size)
		}
	}
}
...
func walletListener(ws *websocket.Conn, out chan<- []byte, done chan<- struct{}, state *models.State) {
	...
	for {
		...
		logDebug("Received message from exchange: %s", string(msg))
...
func main() {
	...
	enableWebUI := flag.Bool("webui", false, "Enable the web interface")
	webPort := flag.String("web-port", "8080", "Port for the web interface")
	...
	// Create trader
	trader := strategy.NewTrader(apiClient, cfg, state, logger, nil)
	trader.OrderManager = orderManager
	trader.PositionManager = positionManager
	...
	var webUI *web_interface.WebUI
	if *enableWebUI || cfg.EnableWebUI {
		go func() {
			logInfo("Starting WebUI on port %s", *webPort)
			webUI = web_interface.NewWebUI(cfg, state)

			// Start the WebSocket broadcast handler and periodic updates
			go webUI.StartPeriodicUpdates()
	...
	drainWallet:
		for {
			select {
			case raw := <-walletChan:
				logDebug("Received private message from exchange: %s", string(raw))
			default:
				break drainWallet
			}
		}
		...
		logDebug("Received public message from exchange: %s", string(raw))
		...
		case strings.HasPrefix(peek.Topic, "kline."):
			...
			if json.Unmarshal(raw, &km) == nil && len(km.Data) > 0 && km.Data[0].Confirm {
				...
				// Update state with the latest close price
				if len(state.Closes) >= 1000 {
					// Keep only the most recent 1000 closes
					state.Closes = state.Closes[len(state.Closes)-999:]
				}
				state.Closes = append(state.Closes, km.Data[0].CloseFloat())

				// If web UI is running, broadcast the update
				if webUI != nil {
					dashboardData := webUI.GetDashboardData()
					webUI.BroadcastUpdate("dashboard_update", dashboardData)
				}
			}
		...
		case sig := <-signals:
			...
			if trader != nil && trader.PositionManager != nil {
				logInfo("Cancelling all active orders before shutdown...")
				trader.PositionManager.CancelAllOrders(cfg.Symbol)
			}
			...
			if pubConn := state.PubPtr.Load(); pubConn != nil {
				if err := pubConn.Close(); err != nil {
					logError("Error closing public WebSocket: %v", err)
				}
			}

			if privConn := state.PrivPtr.Load(); privConn != nil {
				if err := privConn.Close(); err != nil {
					logError("Error closing private WebSocket: %v", err)
				}
			}

			// Close the web UI if it's running
			if webUI != nil {
				logInfo("Shutting down WebUI...")
				// Broadcast shutdown message to all connected clients
				webUI.BroadcastUpdate("system_message", map[string]string{
					"type": "shutdown",
			...
			logInfo("Application shut down completed")
			return
		default:
		}
	}
